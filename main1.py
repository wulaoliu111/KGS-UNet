...
def unwrap_output(output):
    ...
    if isinstance(output, dict):
        ...
        if 'out' in output:    return output['out']

        for k in ['main', 'pred', 'y']:
...
def parse_arguments():
    ...
    parser.add_argument('--seed', type=int, default=421, help='seed')
    ...
    parser.add_argument('--warm_epochs', type=int, default=30, help='epochs for SSA/Mixer warmup')
...
def zero_shot(args, logger, model=None, wandb=None):
    ...
    zero_shot_result = {"zeroshot_loss": avg_meters['val_loss'].avg, "zeroshot_iou": avg_meters['val_iou'].avg,
                        "zeroshot_SE": avg_meters['SE'].avg,
                        "zeroshot_PC": avg_meters['PC'].avg, "zeroshot_F1": avg_meters['F1'].avg,
                        "zeroshot_ACC": avg_meters['ACC'].avg}
...
def train(args, exp_save_dir, writer, logger, model):
    ...
    for epoch_num in tqdm(range(start_epoch, max_epoch), desc='Training Progress'):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if args.do_deeps:
                ...
                ds_w = [0.3, 0.7, 1.0] if (isinstance(outputs, (list, tuple)) and len(outputs) == 3) else None
                loss = deep_supervision_loss(outputs=outputs, label_batch=label_batch, loss_metric=criterion,
                                             weights=ds_w)
                outputs = unwrap_output(outputs)
    ...
    # Save the figure
    plt.savefig(
        f'{exp_save_dir}/{args.model}_{args.train_file_dir}_{args.batch_size}_{args.max_epochs}_{args.seed}_{args.base_lr}_{train_metric_dict["best_iou"]:.4f}.png')

    train_metric_dict = convert_to_numpy(train_metric_dict)
    logger.info(
        f"Training completed. Best IoU: {train_metric_dict['best_iou']}, Best Epoch: {train_metric_dict['best_epoch']}, Best SE: {train_metric_dict['best_iou_withSE']}, Best PC: {train_metric_dict['best_iou_withPC']}, Best F1: {train_metric_dict['best_iou_withF1']}, Best ACC: {train_metric_dict['best_iou_withACC']}")
    logger.info(
        f"Last IoU: {train_metric_dict['last_iou']}, Last SE: {train_metric_dict['last_SE']}, Last PC: {train_metric_dict['last_PC']}, Last F1: {train_metric_dict['last_F1']}, Last ACC: {train_metric_dict['last_ACC']}")
